...
def get_assessment_content() -> dict:
    ...
    return {
        ...
        "problemSolving": {
            ...
            "instructions": "Answer the following questions. For multiple choice, select the best answer. For short answer, keep your response to 1-2 sentences.",
            "questions": [
                {
                    "id": "ps1",
                    "type": "mcq",
                    "questionText": "A scheduled job that usually finishes in 15 minutes has been running for 2 hours. What should you do first?",
                    "options": [
                        {"id": "A", "text": "Restart it"},
                        {"id": "B", "text": "Check logs and resource usage, then decide"},
                        {"id": "C", "text": "Scale up the workers"},
                        {"id": "D", "text": "Notify someone and wait"},
                    ],
                },
                {
                    "id": "ps2",
                    "type": "short_answer",
                    "questionText": "In 1-2 sentences: when would you put a queue between two services instead of one calling the other directly?",
                },
                {
                    "id": "ps3",
                    "type": "mcq",
                    "questionText": "Latency is up ~20% this week. Possible causes: recent deploy, more traffic, or a slow external API. What's the best order of checks?",
                    "options": [
                        {"id": "A", "text": "Traffic → Deploy → External API"},
                        {"id": "B", "text": "Deploy → Traffic → External API"},
                        {"id": "C", "text": "External API → Traffic → Deploy"},
                        {"id": "D", "text": "Deploy → External API → Traffic"},
                    ],
                },
                {
                    "id": "ps4",
                    "type": "short_answer",
                    "questionText": "A teammate wants to rewrite an old script in a new language \"to make it maintainable.\" What 1-2 questions would you ask before agreeing or pushing back?",
                },
                {
                    "id": "ps5",
                    "type": "mcq",
                    "questionText": "Right before a release you have time for only one: run the full test suite (often flaky), or run only tests that touch your changes. What do you do?",
                    "options": [
                        {"id": "A", "text": "Full suite"},
                        {"id": "B", "text": "Only changed code"},
                        {"id": "C", "text": "Quick smoke + tests for your changes"},
                        {"id": "D", "text": "Skip and deploy"},
                    ],
                },
                {
                    "id": "ps6",
                    "type": "mcq",
                    "questionText": "You can add either integration tests for the new feature or a short runbook for the new service. You have time for one.",
                    "options": [
                        {"id": "A", "text": "Tests"},
                        {"id": "B", "text": "Runbook"},
                        {"id": "C", "text": "It depends (explain in one sentence)"},
                    ],
                },
            ],
        },
        ...
        "coding": {
            ...
            "instructions": "Solve the following problem in Python. Your code should read from stdin and print to stdout.",
            "problem": {
                "title": "Two Sum",
                "description": """Given a list of integers and a target sum, find two numbers in the list that add up to the target.

Return the two numbers as a space-separated string. If no such pair exists, return "No solution".

**Input:**
- First line: space-separated integers (the list)
- Second line: the target sum (an integer)

**Output:**
- Two numbers that add up to target, space-separated
- Or "No solution" if no pair exists
...
**Constraints:**
- The list has between 2 and 100 numbers
- Each number is between -1000 and 1000
- There is at most one valid pair""",
                "starterCode": """# Read the list of numbers
...
""",
        ...
        "systemDesign": {
            "title": "Feature Creation – System Design",
            "timeEstimate": "10-15 min",
            "instructions": "Read the feature description and answer the questions below. Keep your total response to about half a page (or a few bullets per part).",
            "prompt": """**Feature: Run and share small code snippets**

Users can submit a short code snippet (e.g. Python), run it in a secure environment, and see the output. They can optionally save and share a link so others can run the same snippet.

In **about half a page** (or a few bullets per part), cover:

1. **Scope** – What's in and out of scope for v1? (1-2 sentences.)

2. **Data flow** – From "user submits code" to "user sees output." List the main pieces and how they connect.

3. **Execution** – How do you run untrusted code safely? What limits (time, memory, network) would you set?

4. **Storage** – What do you store, and how do shareable links map to it?

5. **One trade-off** – Pick one scaling or reliability trade-off for v1 and how you'd revisit it later.

You don't need to name specific tools. We care about clear thinking, not length.""",
        },
    }
